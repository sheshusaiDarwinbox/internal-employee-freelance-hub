...
exports.createUser = (0, session_util_1.sessionHandler)((req, res, session) => __awaiter(void 0, void 0, void 0, function* () {
    const data = zod_util_1.CreateUserSchema.parse(req.body);
    const department = yield department_model_1.DepartmentModel.findOne({ DID: data.DID }, null, {
        session,
    });
    const position = yield position_model_1.PositionModel.findOne({ PID: data.PID }, null, {
        session,
    });
    if (!department || !position)
        throw new Error("Department or Position not found");
    const id = yield (0, counterManager_util_1.generateId)(idCounter_model_1.IDs.EID, session);
    ...
    const [user] = yield userAuth_model_1.User.create([
        Object.assign(Object.assign({}, data), { verified: false, EID: id, password: hashedPassword, doj: new Date() }),
    ], { session });
    const result = yield department_model_1.DepartmentModel.findOneAndUpdate({ DID: data.DID }, { $inc: { teamSize: 1 } }, { session, new: true });
    if (!result)
        throw new Error("Department not updated");
    const resultStatus = yield (0, mail_util_1.sendVerificationEmail)({ EID: id, _id: user._id, password, email: data.email }, session);
    return resultStatus;
}));
